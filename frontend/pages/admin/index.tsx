import { containerStyle, defaultMapOptions, disasterIcons, origin } from "@/components/maps/GoogleMap";
    ...
    return (
        <div className="p-4">
            ...
            <TabsContent value="reports" className="grid gap-2">
                {reports.map((report, i) => (<div key={i} className={`p-4 rounded-md border ${
                    report.status === "PENDING" ? "border-yellow-500 border-2" :
                    report.status === "ACTIVE" ? "border-green-500 border-2" :
                    report.status === "REJECTED" ? "border-red-500 border-2 opacity-50" : ""
                    }`}>
                    <p className="font-semibold text-lg mb-2">{report.description}</p>
